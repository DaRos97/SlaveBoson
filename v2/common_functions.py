...
def checkHessian(P,args):
    ...
    for i in range(len(P)):
        ...
        for j in range(3):
            pp[i] = ptsP[j]
            der.append(totE(pp,args))        #uses at each energy evaluation the best lambda
        for l in range(2):
        ...
        for file in os.listdir(inp.refDirname):     #find file in dir
            ...
            if np.abs(j2-J2) < inp.cutoff_pts and np.abs(j3-J3) < inp.cutoff_pts:               #once found read it
                with open(inp.refDirname+file, 'r') as f:
                    lines = f.readlines()
                N = (len(lines)-1)//4 + 1   #number of ansatze in the file
                for Ans in ansatze:         #ansatze to be computed
                    for i in range(N):
