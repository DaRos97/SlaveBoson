...
def Nk(P,L,args):
    ...
    N[0,1] = J1*b1p_*exp_k(e1)  + J2*b2*exp_k(f1_)
    N[0,2] = J1*b1p*exp_k(e2_)  + J2*b2p_*exp_k(f2_)
    N[0,4] = J1*b1_*exp_k(e1_)  + J2*exp_k(f1)*(b2p if not p1 else b2p_)
    N[0,5] = J1*b1*exp_k(e2)    + J2*exp_k(f2)*(b2_ if not p1 else b2)
    N[1,2] = J1*(b1_*exp_k(e3_) + b1p_*exp_k(e3))
    N[1,3] = J1*b1*exp_k(e1)    + J2*exp_k(f1_)*(b2pi_ if not p1 else b2pi)
    N[1,5] = J2*(b2*exp_k(f3_) + b2p*exp_k(f3))
    N[2,3] = J1*b1_*exp_k(e2_)  + J2*exp_k(f2_)*(b2i if not p1 else b2i_)
    N[2,4] = J2*(b2p_*exp_k(f3_) + b2i_*exp_k(f3))
    N[3,4] = J1*b1pi_*exp_k(e1) + J2*b2*exp_k(f1_)
    N[3,5] = J1*b1p*exp_k(e2_)  + J2*b2pi_*exp_k(f2_)
    N[4,5] = J1*(b1_*exp_k(e3_) + b1pi_*exp_k(e3))
    N[0,0] = J3*exp_k(g3_)*(b3_ if not p1 else b3)
    N[3,3] = J3*exp_k(g3_)*(b3i_ if not p1 else b3i)
    N[1,4] = J3*(b3_*exp_k(g2_)  + b3*exp_k(g2))
    N[2,5] = J3*(b3*exp_k(g1)    + b3i_*exp_k(g1_))
    ####other half square
    N[m+0,m+1] = J1*b1p*exp_k(e1)  + J2*b2_*exp_k(f1_)
    N[m+0,m+2] = J1*b1p_*exp_k(e2_)  + J2*b2p*exp_k(f2_)
    N[m+0,m+4] = J1*b1*exp_k(e1_)  + J2*exp_k(f1)*(b2p_ if not p1 else b2p)
    N[m+0,m+5] = J1*b1_*exp_k(e2)    + J2*exp_k(f2)*(b2 if not p1 else b2_)
    N[m+1,m+2] = J1*(b1*exp_k(e3_) + b1p*exp_k(e3))
    N[m+1,m+3] = J1*b1_*exp_k(e1)    + J2*exp_k(f1_)*(b2pi if not p1 else b2pi_)
    N[m+1,m+5] = J2*(b2_*exp_k(f3_) + b2p_*exp_k(f3))
    N[m+2,m+3] = J1*b1*exp_k(e2_)  + J2*exp_k(f2_)*(b2i_ if not p1 else b2i)
    N[m+2,m+4] = J2*(b2p*exp_k(f3_) + b2i*exp_k(f3))
    N[m+3,m+4] = J1*b1pi*exp_k(e1) + J2*b2_*exp_k(f1_)
    N[m+3,m+5] = J1*b1p_*exp_k(e2_)  + J2*b2pi*exp_k(f2_)
    N[m+4,m+5] = J1*(b1*exp_k(e3_) + b1pi*exp_k(e3))
    N[m+0,m+0] = J3*exp_k(g3_)*(b3 if not p1 else b3_)
    N[m+3,m+3] = J3*exp_k(g3_)*(b3i if not p1 else b3i_)
    ...
    N[1,m+4] = -J3*(a3*exp_k(g2_) - a3*exp_k(g2))
    N[2,m+5] = J3*(a3*exp_k(g1) - a3i*exp_k(g1_))
    N[0,m+0] = -J3*exp_k(g3_)*a3*(-1)**p1
    N[3,m+3] = -J3*exp_k(g3_)*a3i*(-1)**p1
    #not the diagonal
    ...
    N[4,m+1] = J3*(a3*exp_k(g2) - a3*exp_k(g2_))
    N[5,m+2] = -J3*(a3*exp_k(g1_) - a3i*exp_k(g1))
    N[0,m+0] += J3*exp_k(g3)*a3*(-1)**p1
    N[3,m+3] += J3*exp_k(g3)*a3i*(-1)**p1
    #################################### HERMITIAN MATRIX
    for i in range(2*m):
