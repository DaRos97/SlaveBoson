...
def eigs(P,args):
    ...
    N[1,2] = 
    #block up-right
    N[0,m+0] += J3*A3*exp_k(1,0)
    N[1,m+1] += -J3*A3*exp_k(1,1)
    N[2,m+2] += J3*A3*exp_k(0,1)
    N[0,m+1] += J1*A1*(exp_k(0,1)-1) - J2*A2*(exp_k(1,1)+exp_k(-1,0))
    N[0,m+2] += J1*A1*(exp_k(-1,0)-exp_k(0,1)) + J2*A2*(1+exp_k(-1,1))
    N[1,m+2] += J1*A1*(1-exp_k(-1,0)) - J2*A2*(exp_k(0,1)+exp_k(-1,-1))
    N[1,m+0] -= np.conjugate(D[0,m+1])
    N[2,m+0] -= np.conjugate(D[0,m+2])
    N[2,m+1] -= np.conjugate(D[1,m+2])
    #grid of points
