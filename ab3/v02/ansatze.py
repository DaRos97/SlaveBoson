...
def Nk(P,L,args):
    ...
    if ans == 0:     #3x3
        ...
        phiB1, phiB1p, phiB2, phiB2p, phiB3 = (np.pi, np.pi, np.pi, np.pi, np.pi)
        p1 = 0
    elif ans == 2:  #(0,pi)
        A1,A2,B1,B2,B3 = P
        A3 = 0
    ...
    #elif ans == 4: #cuboc1
    #    A1,A2,A3,B1,B2,B3 =....
    #    phiA1p, phiA2, phiA2p, phiA3, phiB1, phiB1p, phiB2, phiB2p, phiB3 =....
    #    p1 = 1
    ################
    ...
    N[0,4] = J1*b1_*exp_k(e1_)  + J2*b2p_*exp_k(f1)
    N[0,5] = J1*b1*exp_k(e2)    + J2*b2*exp_k(f2)
    N[1,2] = J1*(b1_*exp_k(e3_) + b1p_*exp_k(e3))
    N[1,3] = J1*b1*exp_k(e1)    + J2*b2pi*exp_k(f1_)
    N[1,5] = J2*(b2_*exp_k(f3_) + b2p_*exp_k(f3))
    N[2,3] = J1*b1_*exp_k(e2_)  + J2*b2i_*exp_k(f2_)
    ...
    N[0,m+4] = - J1*a1*exp_k(e1_)   -   J2*a2p*exp_k(f1)
    N[0,m+5] =   J1*a1*exp_k(e2)    +   J2*a2*exp_k(f2)
    N[1,m+2] = -J1*(a1*exp_k(e3_) + a1p*exp_k(e3))
    N[1,m+3] =   J1*a1*exp_k(e1)    +   J2*a2pi*exp_k(f1_)
    N[1,m+5] = -J2*(a2*exp_k(f3_) + a2p*exp_k(f3))
    N[2,m+3] = - J1*a1*exp_k(e2_)   -   J2*a2i*exp_k(f2_)
